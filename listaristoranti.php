...
<body>
    <div id="background">
        ...
        <div class="container mt-3 divBackground p-3">
            <div class="row">
                <?php
                    if ($result = $conn->query("SELECT nome, indirizzo, citta FROM ristorante")) {
                        if ($result->num_rows>0) {
                            while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<p class='card-text'><i><b>Indirizzo: </b>" . $row["indirizzo"] . " <br><b>Località: </b>" . $row["citta"] .  "</i></p></div></div></div>";
                                }
                        } else {
